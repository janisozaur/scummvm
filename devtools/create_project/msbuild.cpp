...
void MSBuildProvider::outputProjectSettings(std::ofstream &project, const std::string &name, const BuildSetup &setup, bool isRelease, MSVC_Architecture arch, std::string configuration) {
...
void MSBuildProvider::createBuildProp(const BuildSetup &setup, bool isRelease, MSVC_Architecture arch, std::string configuration) {
	const std::string outputBitness = (arch == MSVC_Architecture::ARCH_X86 ? "32" : "64");

	// TODO(janisozaur): fixes needed
